...
class SelfCheckDetectionTests(unittest.TestCase):
    def test_body_dummy_detected_on_400_frame(self) -> None:
        ok, lines, err = run_body_detection_check(detection.find_best_target, CONFIG)
    ...
    def test_legacy_blob_not_used_as_pass_gate(self) -> None:
        ...
        r_body = detection.find_best_target(
        ...
        r_blob = detection.find_best_target(
    ...
    def test_blob_sanity_lines(self) -> None:
        lines = run_blob_mask_sanity(detection.find_best_target, CONFIG)
